...
export default function Lesson({slug}:{slug:string}) {
  ...
  return <div className="learn-page"><header className="learn-nav"><Link className="brand" href="/" aria-label="知合首页"><span className="brand-mark">知合</span><span className="brand-latin">ZHIHE</span></Link><div className="learn-nav-actions"><Link href="/learn">课程目录</Link><Link href="/checkout">演练订单</Link></div></header>
    {!current && <main className="learn-main"><p role="status">正在加载课程…</p></main>}
    {current?.error && <main className="learn-main"><section className="learn-panel"><p className="eyebrow">关系练习室</p><h1>{current.error === 'locked' ? '这节课需要演练权益。' : current.error === 'missing' ? '没有找到这节课。' : '课程暂时无法加载。'}</h1><p>{current.error === 'locked' ? '第 2–7 课正文由服务端检查权限后提供。你可以完成本地模拟订单开通，不会扣款；已经开通过的话，请使用原浏览器查看订单状态。' : current.error === 'missing' ? '请回课程目录选择已开放的课程。' : '请检查本地服务是否仍在运行，网络恢复后可以重试。'}</p><div className="learn-end-links">{current.error === 'locked' && <Link className="learn-primary" href="/checkout">前往本地演练开通 →</Link>}{current.error === 'network' && <button className="learn-primary" onClick={() => setRetry(value => value+1)}>重新加载</button>}<Link href="/learn">返回课程目录</Link><Link href="/learn/expression">先学免费第 1 课</Link></div></section></main>}
...
function LessonFlow({lesson}:{lesson:CourseLesson}) {
  ...
  return <main className="learn-main"><div className="learn-intro"><p className="eyebrow">第 {String(lesson.number).padStart(2,'0')} 课 / {lesson.number === 1 ? '免费学习' : '本地演练权益'}</p><h1>{lesson.title}</h1><p>{lesson.subtitle} · 约 {lesson.duration} 分钟</p></div>
